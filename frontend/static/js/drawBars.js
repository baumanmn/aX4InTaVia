...
function computeXPos(tokens, chart) {
  function computeFactors(tokens, chart) {
    ...
    if (numAnno === 0 || numNoAnno === 0) {
      ...
      (widthAnno =
        (magnificationFactor * chart.p.tokenExt) /
        (numNoAnno + numAnno * magnificationFactor)),
        (widthNoAnno =
          chart.p.tokenExt / (numNoAnno + numAnno * magnificationFactor));
    }
  ...
  var factors = computeFactors(tokens, chart);
...
function drawTermBorder(chart, width, height, xPos, gap) {
  if (gap) {
    ...
    if (xPos >= chart.p.tokenExt - width)
      return `${tick}, ${width - tick + height}, ${width + height + tick}`;
    //if (xPos >= chart.p.tokenExt - tokenExt) return `${tokenExt}, ${height}, ${tokenExt + height}`;
...
export function drawAtomicDetailBars(chart, tokenRange, lens = false) {
    ...
    (function () {
      ...
      p.children.push({
        ...
        attributes: (function () {
          ...
          a.y = chart.p.wordViewExt + chart.p.wordViewConnectorsExt;
          a.height = chart.p.annotatorBandsExt;
  ...
  //    //     return Math.sqrt(Math.pow(annotatorBandsExt - wordViewConnectorsExt, 2) + Math.pow(xScaleTokenText.bandwidth(), 2));
  //    // })
  ...
  //            + String(annotatorBandsExt - wordViewConnectorsExt) + ")"
  //            + " translate("
  //            + String(2) + ")";
  //    })
  ...
  drawRects(
    (function () {
      ...
      p.group = chart.userAtomicFrames;
      p.data = function (d) {
        //add the user number d (the data of the group-element) to the tokens
        return tokens.map(function (t) {
          return { token: t, user: d };
          ...
          attributes: (function () {
            ...
            a.width = function (d) {
              ...
              if (
                ...
                return chart.p.tokenExt + 2 * chart.p.overlapMargin;
...
export function drawSemiAggregatedDetailBars(chart, tokenRange, lens = false) {
  ...
  chart.p.xScaleTokens_Bins.range([0, chart.p.tokenExt]);
  ...
  drawRects(
    (function () {
      ...
      p.y = chart.p.wordViewExt + chart.p.wordViewConnectorsExt;
      p.height = chart.p.annotatorBandsExt;
    ...
    drawGroupedElements(
      (function () {
          ...
          attributes: (function () {
            ...
            grandChildren.push({
              ...
              attributes: (function () {
                ...
                a.y1 = chart.p.wordViewExt;
                a.x2 = function (d) {
                  return d.xPos + chart.p.xScaleTerms.bandwidth() / 2;
                };
                a.y2 = chart.p.wordViewExt + chart.p.indConnector;
            ...
            grandChildren.push({
              ...
              attributes: (function () {
                ...
                a.y1 = chart.p.wordViewExt + chart.p.indConnector;
                a.x2 = function (d) {
                  return Math.max(
                    d.xPos + chart.p.xScaleTerms.bandwidth() / 2,
            ...
            grandChildren.push({
              ...
              factory: function (selection) {
                ...
                p = {
                  ...
                  y1: chart.p.wordViewExt + chart.p.indConnector,
                  x2: function (d) {
                    return (
                      chart.p.xScaleTokens_Bins(d.id) +
                      chart.p.xScaleTokens_Bins.bandwidth() / 2
        ...
        p.filter = {
          ...
          y: chart.p.wordViewExt - chart.p.xScaleTerms.bandwidth() / 2,
          width: function (d) {
            return (
              (chart.p.globalTermHeight * Math.log(d.localAnnosPerTerm + 1)) /
              Math.log(d.maxGlobalAnnosPerTerm + 1)
...
export function drawAggregatedDetailBars(chart, tokenRange, lens = false) {
  ...
  if (lens) {
    ...
    var tokens = chart.d.tokens.slice(tokenRange[0], tokenRange[1] + 1);
    var bins = computeBins(chart.p.tokenExt, tokens); //re-binning
  ...
  drawRects(
    (function () {
      ...
      p.width = chart.p.xScaleTokens_Bins.bandwidth();
      p.y = chart.p.wordViewExt + chart.p.wordViewConnectorsExt;
      p.height = chart.p.annotatorBandsExt + 100;
...
//HM
function newComputeXPos(tokens, chart, windowWidth) {
  function computeFactors(tokens, chart) {
    //compute the magnification factors for (non-)annotated tokens
    var factors = {},
      magnificationFactor,
    ...
    if (numAnno === 0 || numNoAnno === 0) {
      magnificationFactor = 1;
      widthAnno = windowWidth / Math.max(numAnno, numNoAnno);
      widthNoAnno = widthAnno;
    } else {
      //maximal magnification factor M, the chosen magnification should be in [1,M]
      var maxMagnificationFactor =
        (windowWidth - numNoAnno * chart.p.lowerTokenThreshold) /
        (numAnno * chart.p.lowerTokenThreshold);
      //choose actual magnification factor as a percentage
      magnificationFactor =
        1 + (maxMagnificationFactor - 1) * chart.p.magFactor;
      //width of tokens; if magFactor = 1, then widthNoAnno = LowerTokenThreshold
      (widthAnno =
        (magnificationFactor * windowWidth) /
        (numNoAnno + numAnno * magnificationFactor)),
        (widthNoAnno =
          windowWidth / (numNoAnno + numAnno * magnificationFactor));
    }
...
//(HM)
function newComputeLocalTerms(chart, tokenRange, binning, windowWidth) {
  var b = chart.p.xScaleTerms.bandwidth();
...
//(HM)
// new drawDetailBars
export function newDrawAggregatedDetailBars(
  chart,
  {
    tokenRange,
    windowWidth,
  ...
  //the bins to be drawn
  var tokens = chart.d.tokens.slice(tokenRange[0], tokenRange[1] + 1);
  var bins = computeBins(windowWidth, tokens); //re-binning
  chart.d.detailBins = bins;
  chart.p["binRange_" + windowId] = [bins[0].id, bins[bins.length - 1].id];

  var binMaxima = chart.d.binMaxima; //independent of the new binning

  chart.p.xScaleTokens_Bins = d3
    .scaleBand()
    .range([0, windowWidth])
    .domain(
      bins.map(function (bin) {
  ...
  chart.p.numOfTerms = Math.floor(windowWidth / minTermWidth);
  chart.p.xScaleTerms = d3
    .scaleBand()
    .range([0, windowWidth])
    .domain(d3.range(chart.p.numOfTerms));
  //the tokens/bins & positions for the top-window
  var localTerms = computeLocalTerms(chart, tokenRange, true);
  ...
  drawRects(
    (function () {
      ...
      p.width = chart.p.xScaleTokens_Bins.bandwidth();
      p.y = chart.p.wordViewExt + chart.p.wordViewConnectorsExt;
      p.height = chart.p.annotatorBandsExt - 1;
...
//(HM)
export function newDrawSemiAggregatedDetailBars(
  chart,
  {
    tokenRange,
    windowWidth,
  ...
  chart.p.xScaleTerms = d3
    ...
    .domain(d3.range(chart.p.numOfTerms));
  //the tokens & positions for the top-window
  var localTerms = newComputeLocalTerms(chart, tokenRange, false, windowWidth);
...
//(HM)
export function newDrawAtomicDetailBars(
  chart,
  {
    tokenRange,
    windowWidth,
  ...
  var [xPos, factors] = newComputeXPos(tokens, chart, windowWidth);

  // new
  var xScaleTokenText = d3.scaleBand().range([0, windowWidth]);
      ...
      if (!lens) {
        ...
        p.children.push({
          ...
          attributes: (function () {
            ...
            a.width = xScaleTokenText.bandwidth();
            a.y = 0;
            a.height = chart.p.wordViewExt;
          ...
          attributes: (function () {
            ...
            a.transform = function (d) {
              ...
              return (
                ...
                "," +
                String(
                  xScaleTokenText(d.id) + xScaleTokenText.bandwidth() / 2
                ) +
                "," +
                String(chart.p.wordViewExt) +
                ")" +
          ...
          attributes: (function () {
            ...
            a.width = function (d) {
              ...
              if (
                ...
                return windowWidth + 2 * chart.p.overlapMargin;
              //overlap on the right side
              if (d.tokens.last > tokenRange[1])
                return (
                  factors.widthAnno * (tokenRange[1] - d.tokens.first + 1) -
...
//    }
//  }
export function updateDetailBars(chart, range) {
  // drawDetailBars(chart, [range]);
  //drawDetailBars(chart, [range, [0, 300]]); // example call
  // //Test with  Interval
...
//--------------------------------------JENA: SECOND OVERVIEW-------------------------------------------------------------------------
export function drawSecondOverviewBars(chart, binRange, tokenRange) {
  if (tokenRange[1] < chart.p.tokenExt / chart.p.lowerTokenThreshold) {
    drawSecondViewAtomicOverviewBars(chart, [tokenRange[0], tokenRange[1] - 1]);
  } else {
    if (tokenRange[1] < chart.p.tokenExt) {
      drawSecondViewSemiAggregatedOverviewBars(chart, [
        tokenRange[0],
        tokenRange[1] - 1,
      ]);
    } else {
      drawSecondViewAggregatedOverviewBars(chart, [
        binRange[0],
        binRange[1] - 1,
      ]);
    }
...
export function drawSecondViewAtomicOverviewBars(chart, tokenRange) {
  var tokens = chart.d.tokens.slice(tokenRange[0], tokenRange[1] + 1);
  var [xPos, factors] = computeXPos(tokens, chart);

  chart.overviews[1]["backgroundRects"] //chart.overviewRects2
    .selectAll("rect")
    .data(tokens)
    .enter()
    .append("rect")
...
export function drawSecondViewSemiAggregatedOverviewBars(chart, tokenRange) {
  var tokens = chart.d.tokens.slice(tokenRange[0], tokenRange[1] + 1);
  const tempScale = d3.scaleBand().range([0, chart.p.tokenExt]);
  tempScale.domain(
    tokens.map(function (token) {
      return token.id;
    })
  );
  tempScale.domain().map(tempScale);
  chart.overviews[1]["backgroundRects"] //chart.overviewRects2
    .selectAll("rect")
    .data(tokens)
    .enter()
    ...
    .attr("width", tempScale.bandwidth())
    //.attr("tokenExt", chart.p.xScaleTokens_Bins.bandwidth())
    .attr("y", 0)
    .attr("height", function (d) {
      return (
        chart.p.overviewExt *
...
export function drawSecondViewAggregatedOverviewBars(chart, binRange) {
  var bins = chart.d.bins.slice(binRange[0], binRange[1] + 1);
  bins.maxSize = chart.d.bins.maxSize; //this was lost because of the slicing
  var binMaxima = chart.d.binMaxima;
  /**
   * JENA: temporary scale to adjust to partitioning
   */
  const tempScale = d3.scaleBand().range([0, chart.p.tokenExt]);
  tempScale.domain(
    //
...
//----------------------------JENA: THIRD OVERVIEW-------------------------
export function drawThirdOverviewBars(chart, binRange, tokenRange) {
  if (tokenRange[1] < chart.p.tokenExt / chart.p.lowerTokenThreshold) {
    drawThirdViewAtomicOverviewBars(chart, [tokenRange[0], tokenRange[1] - 1]);
  } else {
    if (tokenRange[1] < chart.p.tokenExt) {
      drawThirdViewSemiAggregatedOverviewBars(chart, [
        tokenRange[0],
        tokenRange[1] - 1,
      ]);
    } else {
      drawThirdViewAggregatedOverviewBars(chart, [
        binRange[0],
        binRange[1] - 1,
      ]);
    }
...
export function drawThirdViewAtomicOverviewBars(chart, tokenRange) {
  var tokens = chart.d.tokens.slice(tokenRange[0], tokenRange[1] + 1);
  var [xPos, factors] = computeXPos(tokens, chart);

  chart.overviews[2]["backgroundRects"] //chart.overviewRects3
    .selectAll("rect")
    .data(tokens)
    .enter()
    .append("rect")
...
export function drawThirdViewSemiAggregatedOverviewBars(chart, tokenRange) {
  var tokens = chart.d.tokens.slice(tokenRange[0], tokenRange[1] + 1);
  const tempScale = d3.scaleBand().range([0, chart.p.tokenExt]);
  tempScale.domain(
    tokens.map(function (token) {
...
export function drawThirdViewAggregatedOverviewBars(chart, binRange) {
  var bins = chart.d.bins.slice(binRange[0], binRange[1] + 1);
  bins.maxSize = chart.d.bins.maxSize; //this was lost because of the slicing
  var binMaxima = chart.d.binMaxima;
  /**
   * JENA: temporary scale for stacked view
   */
  const tempScale = d3.scaleBand().range([0, chart.p.tokenExt]);
  tempScale.domain(
    //
