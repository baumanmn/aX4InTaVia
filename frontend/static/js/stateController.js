...
export var StateController = (function () {
        ...
        case "semiAggregated": {
          for (var i = 0; i < tempStorage.length; i++) {
            ...
            if (
              ...
              //     var HeadTermIndicator = new ElemAddress("HeadTermIndicator", termId);
  ...
  // TODO Write/delete Private for all
  // TODO Refactor normal function names
  ...
  function highlightTextAnno(elemAddress, mod = true) {
    ...
    // scrollToTarget(annoData.tokens.last, "nearest");
  }
      ...
      if (mod) {
        if (chart.modeDetail === "aggregated") {
          ...
          d3.select("#detailBgTB_g")
            ...
            .attr("width", binSteps)
            .attr("height", chart.p.annotatorBandsExt)
  ...
  function highlightSemiAggregatedChartNotAnnotatedToken(
    ...
    if (mod) {
      if (chart.modeDetail === "semiAggregated") {
        ...
        d3.selectAll("#userTokens_BinsG" + elemAddres.user)
          .append("rect")
          .classed("noAnnoSemiAggregated_t_b", true)
